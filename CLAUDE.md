...
- `action_plugins/ssh-reconnect.py`: all the logic. It runs **on the controller**, not on the target:
  - It builds a shell pipeline (`ps -ef | grep sshd... | awk ... sudo -n kill -9 PID | sh`) and runs it by starting a separate local `ssh -tt -n -S none <host>` subprocess. It deliberately bypasses Ansible's connection plugin (`-S none` avoids the ControlMaster socket, so it does not kill its own multiplexed connection).
  - Only `self._connection.host` is passed. Ansible's ssh user, port, key and other connection vars are **not** used, so the call relies on the controller's `~/.ssh/config` and defaults.
